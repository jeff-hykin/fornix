...
in
    ...
    definitions.mainPackages.mkShell {
        ...
        shellHook = ''
            ...
            # save the cpp lib path
            printf '%s' '${definitions.mainRepo.stdenv.cc.cc.lib}' > "$NIX_PROJECTR_ENV_PASSTHROUGH/NIX_CPP_LIB_PATH"
            printf '%s' 'nixpkgs=${definitions.mainRepo}:.' > "$NIX_PROJECTR_ENV_PASSTHROUGH/NIX_PATH"
            
            echo 'HOME: '"$HOME"
            echo 'PROJECTR_HOME: '"$PROJECTR_HOME"
            echo "DEBUGGING: 029r8u24jf"
