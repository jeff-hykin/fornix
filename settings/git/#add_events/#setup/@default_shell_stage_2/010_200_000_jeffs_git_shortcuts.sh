...
git_checkout_pr () {
    if [ -z "$1" ]
    then
        echo "whats the github # of the pull request? (like issue number)"
        read pr_id
    else
        pr_id="$1"
    fi
    temp_branch_name="@__pull_request__"
    
    git push origin --delete "$temp_branch_name" 2>/dev/null
    git branch -D "$temp_branch_name"
    git fetch origin "pull/$pr_id/head:$temp_branch_name"
    git checkout "$temp_branch_name"
    
    unset temp_branch_name
    unset pr_id
}
